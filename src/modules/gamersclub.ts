...
async function getGamersClubImpl(url: URL, league: League, live: boolean) {
...
export async function getGamersClub(tournament: number, league: League): Promise<Match[]> {
  ...
  return (await getGamersClubImpl(liveUrl, league, true)).concat((await getGamersClubImpl(upUrl, league, false)));
}